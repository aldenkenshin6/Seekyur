...
const AnalystAlertLogs = () => {
    const { alerts, loading } = useSocket();
        ...
        <div className="dashboard-layout analyst-theme">
            ...
            <div className="main-content">
                ...
                <div className="incidents-content">
                    <div className="alert-logs-title-row">
                    ...
                    <div className="alerts-stat-row">
                        <div className="alert-stat-card">
                            <h3 className="alert-stat-value" style={{ color: 'var(--critical-color)' }}>{criticalCount}</h3>
                            <p className="alert-stat-label">Critical</p>
                        </div>
                        <div className="alert-stat-card">
                            <h3 className="alert-stat-value" style={{ color: 'var(--high-color)' }}>{highCount}</h3>
                            <p className="alert-stat-label">High</p>
                        </div>
                        <div className="alert-stat-card">
                            <h3 className="alert-stat-value" style={{ color: 'var(--medium-color)' }}>{mediumCount}</h3>
                            <p className="alert-stat-label">Medium</p>
                        </div>
                        <div className="alert-stat-card">
                            <h3 className="alert-stat-value" style={{ color: 'var(--low-color)' }}>{lowCount}</h3>
                                ...
                                {paginatedAlerts.map(a => {
                                    ...
                                    return (
                                        <tr key={a._id}>
                                            ...
                                            <td className="updated-time">{formatRelativeTime(a.createdAt)}</td>
                                        </tr>
                                    );
                                })}
                                {filteredAlerts.length === 0 && !loading && (
                                    <tr>
                                        <td colSpan="6" style={{ textAlign: 'center', padding: '30px', color: '#64748b' }}>No alerts captured.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
...
export default AnalystAlertLogs;
