...
import AnalystAuthLogs from './Analyst/AuthLogs';

const DefaultAuthLogs = () => {
            ...
            <div className="main-content">
                ...
                <div className="incidents-content">
                    ...
                    <div className="authlogs-stat-row">
                        ...
                        <div className="authlogs-stat-card">
                            <h3 className="authlogs-stat-value" style={{ color: '#10b981' }}>{successCount}</h3>
                            <p className="authlogs-stat-label">Successful</p>
                        </div>
                        <div className="authlogs-stat-card">
                            <h3 className="authlogs-stat-value" style={{ color: '#ef4444' }}>{failedCount}</h3>
                            <p className="authlogs-stat-label">Failed Attempts</p>
                        </div>
                        <div className="authlogs-stat-card">
                            <h3 className="authlogs-stat-value" style={{ color: '#f59e0b' }}>{uniqueFailedIPs}</h3>
                    ...
                    <div className="live-feed-banner" style={{ justifyContent: 'space-between', borderBottom: '1px solid #1e293b', paddingBottom: '12px', marginBottom: '0' }}>
                        <div style={{ color: '#e2e8f0', fontSize: '13px', fontWeight: '600' }}>Authentication Events</div>
                        <div style={{ color: '#64748b' }}>showing {filteredLogs.length} of {totalCount} entries</div>
                    </div>
                            ...
                            <tbody>
                                {paginatedLogs.map(log => {
                                    ...
                                    return (
                                        <tr key={log._id}>
                                            <td>
                                                <span className={`severity-pill ${resLower === 'success' ? 'low' : 'critical'}`}>
...
const AuthLogs = () => {
    ...
    if (user?.role === 'SOC Analyst' || user?.role === 'Incident Responder') {
        return <AnalystAuthLogs />;
    }
    return <DefaultAuthLogs />;
};
